...
static esp_err_t result_handler(httpd_req_t *req)
{
    ...
    xQueuePeek(xQueueResultI, &result, 0);

    char *p = json_response;
    *p++ = '{';
    p += sprintf(p, "\"result\":\"%f\"", result);
    *p++ = '}';
    *p++ = 0;
...
{
    extern const unsigned char index_ov2640_html_gz_start[] asm("_binary_index_html_gz_start");
    extern const unsigned char index_ov2640_html_gz_end[] asm("_binary_index_html_gz_end");
